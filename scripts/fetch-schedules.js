...
async function fetchSchedule(browser, url) {
  ...
  try {
    ...
    const schedule = await context.evaluate((currentWorkerName) => {
      ...
      // ‚îÄ‚îÄ Extract productions from day cells ‚îÄ‚îÄ
      ...
      dayCells.forEach((cell, dayIndex) => {
        ...
        eventDivs.forEach((eventDiv) => {
          ...
          const studioMatch = rawProductionName.match(
            /(?:◊ê◊ï◊ú◊§◊ü|◊°◊ò◊ï◊ì◊ô◊ï|studio|st\.?)\s*(\d+\w?)/i
          );
    ...
    // ‚îÄ‚îÄ Phase 2: Click each production to get detailed crew with phone numbers ‚îÄ‚îÄ
            ...
            setTimeout(() => {
              try {
                ...
                for (const table of tables) {
                  ...
                  if (!headerText.includes('◊™◊§◊ß◊ô◊ì') && !headerText.includes('◊©◊ù')) continue;
                  ...
                  for (let r = 1; r < rows.length; r++) {
                    ...
                    // ◊™◊§◊ß◊ô◊ì (role) | ◊©◊ù (name) | ◊§◊®◊ò◊ô◊ù (roleDetail) | ◊û◊°' (phone) | ◊©◊¢◊ï◊™ (times)
                    ...
                    if (cells.length >= 5) {
                      ...
                      const timeMatch = (cellTexts[4] || '').match(/(\d{1,2}:\d{2})\s*[-‚Äì]\s*(\d{1,2}:\d{2})/);
                      if (timeMatch) {
                ...
                if (popupTitle) {
                  ...
                  const stMatch = titleText.match(/(?:◊ê◊ï◊ú◊§◊ü|◊°◊ò◊ï◊ì◊ô◊ï)\s*\d+\w?/i);
...
function normalizeName(name) {
  ...
  let cleaned = name
    // Remove role prefixes like "ˆÈÏÂÌ: " "Ò‡Â„: " etc
    .replace(/^[\u05d0-\u05ea]+:\s*/u, '')
    // Remove role suffixes separated by dash
    .replace(/\s*[-ñ]\s*[\u05d0-\u05ea\s]+$/u, '')
    .trim()
    .replace(/\s+/g, ' ');
  ...
  while (changed) {
    ...
    for (const phrase of rolePhrases) {
      ...
      const suffix = new RegExp(`\\s+${phrase}const { initializeApp, cert } = require('firebase-admin/app');
const { getFirestore, FieldValue } = require('firebase-admin/firestore');
const puppeteer = require('puppeteer-core');
const chromium = require('@sparticuz/chromium');
...
, 'u');
  ...
  cleaned = cleaned.replace(/^[ñ-]\\s*/u, '').replace(/\\s*[ñ-]$/u, '').trim();
