...
let lastFetch = 0;
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes (shorter for more freshness)

// RSS feed sources - Israeli media & entertainment
const RSS_SOURCES = [
  {
    name: 'Ynet - חדשות',
...
async function fetchRssFeed(source: typeof RSS_SOURCES[0]): Promise<RssNewsItem[]> {
  try {
    ...
    console.log(`RSS error for ${source.name}:`, err instanceof Error ? err.message : 'unknown');
    return [];
  }
}
