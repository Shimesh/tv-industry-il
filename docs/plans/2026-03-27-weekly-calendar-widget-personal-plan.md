# Weekly Calendar Widget — Personal Schedule Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Upgrade WeeklyCalendarWidget to show the user's personal shifts in orange and open a day-detail popup on click.

**Architecture:** Widget reads localStorage cache instantly (Phase 1), then silently fetches Firestore `productions/global/weeks/{weekId}/productions` in background (Phase 2). User identity matched via `isCurrentUserShift`, crew name, or crew phone. Clicking a day with productions opens an inline popup showing all that day's productions, with user's own shifts highlighted orange.

**Tech Stack:** React 19, Next.js App Router, Firebase Firestore SDK, Tailwind CSS 4, `useAuth` from AuthContext, `lucide-react` icons.

---

### Task 1: Rewrite WeeklyCalendarWidget with personal shift detection

**Files:**
- Modify: `src/components/WeeklyCalendarWidget.tsx`

**Context:**
- `useAuth()` is from `@/contexts/AuthContext` — provides `profile.displayName`, `profile.phone`, `user`
- Firestore path: `productions/global/weeks/{weekId}/productions` (subcollection)
- `getWeekId(todayStr)` returns the Sunday of the current week as `YYYY-MM-DD`
- `normalizePhone` is from `@/lib/crewNormalization`
- Crew member fields: `name`, `normalizedName`, `phone`, `normalizedPhone`, `role`, `roleDetail`, `startTime`, `endTime`
- `db` is from `@/lib/firebase`
  ...
  return (
    <>
      ...
      >
        ...
        {/* Footer — only when no cache and not loading */}
...
```bash
cd C:\Users\User\Desktop\claude\tv-industry-il && npx next build 2>&1 | grep -E "(error|Error|✓ Compiled)"
```

Expected: `✓ Compiled successfully`

**Step 3: Start dev server and check homepage visually**

Open `http://localhost:3000/` and scroll to the widget. Verify:
- [ ] Widget renders without crashing
- [ ] 7 day columns show
- [ ] Today's column has purple circle on date number
- [ ] Header says "לוח הפקות השבוע"
- [ ] "לכל ההפקות" link is present

**Step 4: Verify popup opens on a day with productions (if cache exists)**

If productions are loaded:
- [ ] Days with productions show name chips
- [ ] Clicking a day with productions opens the popup
- [ ] Popup shows production name, time, studio
- [ ] ESC closes popup
- [ ] Clicking backdrop closes popup
- [ ] "לצפייה בלוח המלא" navigates to /productions

**Step 5: Commit**

```bash
git add src/components/WeeklyCalendarWidget.tsx docs/plans/
git commit -m "feat: weekly calendar widget with personal shifts and day popup"
```
