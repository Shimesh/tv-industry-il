...
| Crew modal shows empty / no crew members despite crew_phones being populated | `buildCrewIdentity` returned `identityKey = null` when `normalizedName.length < 2`, even when phone was valid. With swapped name/role data (from old parser bug), `crew_list[].name` = a role word like "צילום". `normalizeContactName("צילום")` strips role words → `""` → length 0 < 2 → `identityKey = null` → `deduplicateCrewEntries` skips all such entries → crew modal appears empty. | Fixed (v2.8.162): `buildCrewIdentity` now returns `identityKey = normalizedPhone` when name is too short but phone is valid. ShowEmp6 appname typo `HSiLWeb` → `HsILWeb` also fixed (was causing dept view fetch to fail). After resync with v2.8.161+ parser, names are correct and this fallback is not needed. |
